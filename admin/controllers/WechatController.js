...
var UserSummary = mongoose.model('UserSummary');
var UserCumulate = mongoose.model('UserCumulate');
...
var errcode = require('../../public/errcode.json');
...
exports.list = function (req, res) {
  ...
  ChannelWechat.find(query).populate('qrcode').exec(function (err, wechatList) {
    res.render('wechat/wechatList', {
...
exports.datatable = function (req, res) {
  ChannelWechat.dataTable(req.query, {
    conditions: {
...
exports.add = function (req, res) {
  ChannelWechat.getAuthWechat(req.session.channel._id, function (err, wechats) {
    if (err) console.error(err);
    console.log("***************** getAuthWechat", wechats);
    res.render('wechat/wechatForm', {
      info: new ChannelWechat(),
...
exports.edit = function (req, res) {
  ...
  ChannelWechat.findById(id).populate('logo qrcode').exec(function (err, result) {
    if (err) {
      console.log(err);
      req.flash('error', '公众号不存在，可能已被删除!');
      res.redirect('/wechat');
    } else {
      ChannelWechat.getAuthWechat(result.channel, function (err, wechats) {
        if (err) console.error(err);
        console.log("***************** getAuthWechat", wechats);
        res.render('wechat/wechatForm', {
          info: result,
...
exports.save = function (req, res) {
  ...
  var saveOrUpdate = function (id, req, callback) {
    if (!id) {
      var wechat = new ChannelWechat(req.body);
      ...
      var json = req.body;
      json.checked = true;
      ChannelWechat.findOneAndUpdate({
        '_id': id
      }, json, {
          new: true
        }, function (err, result) {
  ...
  async.waterfall([(cb) => {
    ...
    api.getAccessToken((e, o) => {
      if (e) {
        cb({ "message": "'不合法的AppID，请开发者检查AppID的正确性，避免异常字符，注意大小写'" }, null)
      } else {
        cb(null, null)
      }
    })
        ...
        imgUtil.thumbnail(fs, 150, function (ffs) {
          ...
          fileUtl.saveFiles(ffs, function (err, files) {
            ...
            saveOrUpdate(id, req, (e, o) => {
              ...
              ChannelWechat.findById(o._id, function (err, wechat) {
...
exports.clearToken = function (req, res) {
  ...
  if (id) {
    ChannelWechat.findById(id, function (err, wechat) {
      if (err) {
        console.error(err);
...
exports.getMedia = function (req, res) {
  ...
  var appsecret = req.session.wechat.appsecret;
  var mutil = new mediaUtil(appid, appsecret);
  WechatMedia.find({ appid: appid, type: type }).exec((err, result) => {
    if (!err) {
...
exports.sendmsg = function (req, res) {
  ...
  ChannelWechat.findOne({ "_id": wechat }, (err, cw) => {
    var type = cw.type;
    //认证订阅号
    if (type == '2') {
      //检查当天
      SendMsgCount.findOne({ "wechat": wechat, 'dateStr': moment().format('YYYYMMDD') }, (e, o) => {
        if (o) {
          if (o.dayCount == 0) {
            res.render('wechat/sendmsg/index', { 'sendFlag': '0', 'msg': '你今天还能群发 1 条消息' });
          } else {
            res.render('wechat/sendmsg/index', { 'sendFlag': '1', 'msg': '你今天还能群发 0 条消息' });
          }
        } else {
          SendMsgCount.initData(type, wechat, (err, obj) => {
            res.render('wechat/sendmsg/index', { 'sendFlag': '1', 'msg': '你今天还能群发 1 条消息' });
          })
        }
      })
    } else if (type == '4') {
      SendMsgCount.findOne({ "wechat": wechat, 'yearMonth': moment().format('YYYYMM') }, (e, o) => {
        if (o) {
          if (o.count > 0) {
            res.render('wechat/sendmsg/index', { 'sendFlag': '1', 'msg': '您本月还能群发 ' + o.count + ' 条消息' });
          } else {
            res.render('wechat/sendmsg/index', { 'sendFlag': '0', 'msg': '您本月还能群发 0 条消息' });
          }
        } else {
          SendMsgCount.initData(type, wechat, (err, obj) => {
            res.render('wechat/sendmsg/index', { 'sendFlag': '1', 'msg': '您本月还能群发 4 条消息' });
          })
        }
      });
    } else { //未认证的订阅号或服务号
      res.render('wechat/sendmsg/index', { 'sendFlag': '0', 'msg': '未认证公众号不能群发消息' });
    }
  });
};
...
exports.autoreply = function (req, res) {
  WechatReply.findOne({
    ...
    res.render('wechat/autoreply/autoreply', {
      wechatId: req.session.wechat._id,
...
exports.designer = function (req, res) {
  ...
  menu.getMenu(function (err, menus) {
    ...
    res.render('wechat/menu/designer', { menus: JSON.stringify(menus.menu) });
  });
};
...
exports.wechatFansGroup = function (req, res) {
  ...
  sumUserCumulate(dateStr, wid, function (uc) {
    ...
    WechatFans.count({ "channelWechat": wid, "createdAt": { $gte: startTime } }, function (e, total) {
      json.cumulate_user = uc.cumulate_user + total;//当天累计关注
      json.addFans = total;//今日新关注
      WechatFans.count({ "channelWechat": wid, "createdAt": { $gte: startTime }, "flag": true }, function (e, newFans) {
...
function sumUserSummary(dateStr, wechatId, cb) {
  ...
  UserSummary.find({ "ref_date": dateStr, "channelWechat": wechatId }, function (err, result) {
...
function sumUserCumulate(dateStr, wechatId, cb) {
  ...
  UserCumulate.find({ "ref_date": dateStr, "channelWechat": wechatId }, function (err, result) {
    if (err || result.length == 0) {
      return cb(uc);
    } else {
      result.forEach(function (data) {
...
exports.groupWechatFans = function (req, res) {
  ...
  var query = { "ref_date": { $gte: startDate, $lte: endDate }, "channelWechat": req.session.wechat._id };
  if (userSource) {
