  ...
  xml2js.parseString(req.body, function (err, result) {
    ...
    if (event) {
      if (event == 'subscribe' && eventKey == '') {
        ...
        getWechatInfo(openid, originalId, null, function () {
          subscribe(openid, originalId, res);
        });
      } else if (event == 'unsubscribe') {
        unsubscribe(openid, res);
      } else if ((event == 'SCAN' || event == "subscribe") && eventKey) {
        getWechatInfo(openid, originalId, ticket, function () {
          //根据二维码对应的标签给粉丝打标签
...
function mergeTags(tags, tagList){
  ...
  if(tags.length + tmpTags.length <=3){
...
function brandingCount(ticket) {
  WechatBranding.update({"ticket": ticket},
    {$inc: {"scanTimes": 1}},
    {"upsert": false, "multi": false}, function (e, o) {
...
function unsubscribe(openid, res) {
  WechatFans.update({"openid": openid}, {"flag": false}, {"upsert": false, "multi": false}, function (e, o) {
  });
  res.end("success");
}

function getWechatInfo(openid, originalId, ticket, cb) {
  ChannelWechat.findOne({"originalId": originalId}, function (e, o) {
    if (e || o == null) {
      console.log("公众号不存在");
      return;
    } else {
      var api = new wechatApiUtil(o.appid, o.appsecret).getWechatApi();
      ...
      api.getUser(openid, function (err, result) {
        if (!err) {
          ...
          result.channelWechat = o._id;//渠道公众号
          //只用于新用户,老用户不可用
          WechatFans.findOne({"openid": openid},(er, ob)=>{
            if(ob == null || (ob && ob.scanCount == 1 && !result.phoneFareFlag) || (result.phoneFareFlag && result.phoneFareFlag < 2)){
              result.phoneFareFlag = 1;//新用户送5元话费标记
            }
            //增加标示
            WechatBranding.findOne({"ticket": ticket}, function (e, wb) {
              if (wb != null) {
                //记录推广人员的标识码，粉丝归类到此推广人员下
                result.identifyNo = wb.identifyNo;
              }
              //只有unionid和wid的(被推广用户打开分享页，关注不带参二维码),设置推广码
              if(result.unionid){
                WechatFans.findOne({"unionid": result.unionid, "channelWechat": o._id}, function (err, wf) {
                  //没有openid的新用户统计次数才+1.
                  if (wf != null) {
                    if(!wf.openid){
                      WechatBranding.update({"user.employeeNo": wf.identifyNo},
                        {$inc: {"gainFans": 1, "scanTimes": 1}},
                        {"upsert": false, "multi": false}, function (e, o) {
                          console.log("have e fans and scanTimes add 1");
                        });
                    }
                    //根据unionid补充粉丝数据
                    wf.openid = result.openid;
                    wf.flag = true;
                    wf.nickname = result.nickname;
                    wf.sex = result.sex;
                    wf.language = result.language;
                    wf.city = result.city;
                    wf.province = result.province;
                    wf.country = result.country;
                    wf.headimgurl = result.headimgurl;
                    wf.remark = result.remark;
                    wf.subscribe = result.subscribe;
                    wf.subscribe_time = result.subscribe_time
                    if(wf.scanCount){
                      wf.scanCount += 1;
                    }else{
                      wf.scanCount = 1;
                    }
                    if((wf.scanCount == 1 && !wf.phoneFareFlag) || (wf.phoneFareFlag && wf.phoneFareFlag < 2)){
                      wf.phoneFareFlag = 1;
                    }
                    //2.借助服务号认证，补充粉丝数据
                    wf.save(function(){
                    ...
                    //1.首次公众号关注 || 取消关注再关注
                    WechatFans.findAndSave(result, function () {
                      return cb();
                    });
                  }
                });
