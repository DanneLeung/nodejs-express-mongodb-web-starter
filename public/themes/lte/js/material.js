define(['jquery', 'underscore', 'bootstrap', 'jquery.wookmark', 'jquery.jplayer'], function ($, _) {
  var material = {
    ...
    'localPage': function (type, page) {
      ...
      var url = '/wechat/media/getMedia?type=' + type;
      if(type == 'wxcard') {
      ...
      $.getJSON(url, { 'page': page }, function (data) {
        ...
        if(!_.isEmpty(data.meg)) {
          $this.modalobj.find('#btn-select').show();
          $content.data('attachment', data.meg);
          $content.empty();
          var Dialog = type + 'Dialog';
          $content.html(_.template($this.buildHtml()[Dialog])({ items: data.meg }));
          //if(type == 'news') {
          //	setTimeout(function(){
          //		$('.water').wookmark({
          //			align: 'center',
      ...
      $this.modalobj.find('.modal-footer .btn-primary').unbind('click').click(function () {
        ...
        $content.find('.checkedMedia').each(function () {
          ...
          attachment.push($(this).data('media'));
        });
    ...
    'buildHtml': function () {
      ...
      dialog['imageDialog'] = '<ul class="img-list clearfix">\n' +
        ...
        '<div class="checkMedia" data-media="<%=item.media_id%>" data-type="image" data-attachid="<%=i%>">' +
        '<%i++;%>\n' +
        '	<li class="img-item" style="padding:5px">\n' +
        '		<div class="img-container" style="background-image: url(\'/wechat/media/getMaterial/<%=item.media_id%>\');">\n' +
      ...
      dialog['voiceDialog'] = '<table class="table table-hover table-bordered" style="margin-bottom:0">' +
        ...
        '										<a href="javascript:;" class="btn btn-default btn-sm checkMedia" data-media="<%=item.media_id%>" data-type="voice" data-attachid="<%=i%>">选取</a>' +
        '									</div>' +
        '								</td>' +
        '							</tr>' +
        '							<%i++;%>\n' +
      ...
      dialog['videoDialog'] = '<table class="table table-hover table-bordered" style="margin-bottom:0">' +
        ...
        '								<td align="right">' +
        '									<div class="btn-group">' +
        '										<a href="javascript:;" class="btn btn-default btn-sm checkMedia" data-media="<%=item.media_id%>" data-type="video" data-attachid="<%=i%>">选取</a>' +
        '									</div>' +
        '								</td>' +
        '							</tr>' +
        '							<%i++;%>\n' +
      ...
      dialog['wxcardDialog'] = '<table class="table table-hover table-bordered">\n' +
        ...
        '						<td><a href="javascript:;" class="btn btn-default btn-sm checkMedia" data-title="<%=item.title%>" data-type="wxcard" data-media="<%=item.card_id%>" data-attachid="<%=i%>">选取</a></td>' +
        '					</tr>' +
        '					<%i++;%>\n' +
        '					<%});%>' +
        '				</tbody>' +
      ...
      dialog['newsDialog'] = '<%var items = _.sortBy(items, function(item) {return -item.update_time;});%>' +
        '					<%var i = 0;%>\n' +
        '					<%_.each(items, function(item) {%> \n' +
        '					<div class="col-md-4 col-md-4 col-md-4 water" style="">' +
        '						<div class="panel-group checkMedia" data-media="<%=item.media_id%>" data-type="news" data-attachid="<%=i%>">' +
        '							<%var index = 0;%>\n' +
        '							<%i++;%>\n' +
        '							<%_.each(item.content.news_item, function(data) {%>\n' +
        '								<%index++;%>\n' +
        ...
        '											<img src="/wechat/media/getMaterial/<%=data.thumb_media_id%>">' +
