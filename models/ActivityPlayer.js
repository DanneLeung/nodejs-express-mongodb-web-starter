...
var ActivityPlayerSchema = new Schema({
  ...
  shareUpdateAt: {type: Date, default: new Date()}, //分享次数
  ...
  timesUpdateAt: {type: Date, default: new Date()}, //次数更新时间
...
ActivityPlayerSchema.statics.pushAwards = function (user, awardInfo, done) {
  ...
  mongoose.model('ActivityPlayer').findOne(q, function (err, player) {
    ...
    Activities.findById(awardInfo.activityId, function (err, activities) {
      ...
      //活动关联中库存剩余量
      var awards = activities.awards, level = 0;
          ...
          async.series([function(cb2) {
            ...
            Activities.findOneAndUpdate({_id: awardInfo.activityId}, {$inc: {__v: 1}}, function (e, o) {
              ...
              if (o) {
                ...
                var v = o.__v;
                v += 1;
                Activities.findOneAndUpdate({_id: awardInfo.activityId, __v: v}, {"awards": awards}, function (err, a) {
                  if (err) {
                    return cb2('领奖出错啦！', null);
                  }
                  return cb2(null, a);
  ...
  function checkMobile (mobile, cb){
    httpUtil.get("http://if.dahanbank.cn/FCGetAttribution", {"mobile": mobile}, function(err, obj){
      if(err || !obj){
