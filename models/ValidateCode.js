 ...
 * @type {Schema}
 */
var ValidateCodeSchema = new Schema({
  ...
  usedgAt: { type: Date } //使用时间
}, { timestamps: {} });

ValidateCodeSchema.methods = {
  /**
   * 生成6位随机数验证码
   * @param len
   * @returns {string}
   */
  genActivateCode: function (len) {
    if(!len) {
      len = 6;
    }
    var code = '';
  ...
  sendToMobile: function (mobile, type, len, ip, callback) {
    ...
    //检查手机号，当天不能发送超过10条短信
    ActivateCode.count({ "mobile": mobile, "type": type, "createdAt": { $gt: nowDate } }, (e, o) => {
      ...
      if(o && o >= 10) {
        ...
        ActivateCode.findOne({ mobile: mobile, expireAt: { $gt: now }, used: false, type: type }, function (err, obj) {
          if(err) {
            console.log(err);
            ...
            if(diff < 60) {
              ...
              var code = new ActivateCode().genActivateCode(len);
              var dnow = new Date();
              //存手机号和验证码到数据库
              var ActivateCode = new ActivateCode({
                mobile: mobile,
                type: type,
                code: code,
                ip: ip,
              ...
              ActivateCode.save(function (err, result) {
                if(err) {
            ...
            ActivateCode.update({ "mobile": mobile, "type": type, "used": false }, { "used": true }, {
              multi: true,
              upsert: false
            }, function (e, o) {
              console.log(e, o);
  ...
  modifyUsed: function (mobile, type, code, callback) {
    ActivateCode.findOneAndUpdate({ mobile: mobile, used: false, type: type, code: code }, { "used": true }, function (e, o) {
      if(e) {
        setTimeout(function () {
          ActivateCode.findOneAndUpdate({ mobile: mobile, used: false, type: type, code: code }, { "used": true }, function (e, o) {
  ...
  checkExpireAt: function (mobile, type, callback) {
    ActivateCode.count({ mobile: mobile, expireAt: { $gt: new Date() }, used: false, type: type }, function (err, count) {
      if(err || count == 0) {
...
var ActivateCode = mongoose.model('ActivateCode', ValidateCodeSchema)