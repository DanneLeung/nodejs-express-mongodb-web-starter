...
var ActivitiesSchema = mongoose.Schema({
  ...
  startTime: {type: Date, required: false, default: new Date()}, //开始时间
  endTime: {type: Date, required: false, default: new Date()}, //结束时间
  ...
  whitelist: {type: ObjectId, ref: "WhiteList"}, //使用的白名单
 ...
 * @param done callback
 */
ActivitiesSchema.statics.take = function (id, answer, aid, user, done) {
    ...
    Stats.findOne({activity: id}).populate('activity').exec(function (err, stats) {
      ...
      async.waterfall([function (cb) {
        ...
        if (!player) {
          ...
          player = new Player({activity: id, user: user, timesUpdateAt: new Date(), weekNth: moment().format('w')});
