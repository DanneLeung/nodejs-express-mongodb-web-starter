 ...
 * @param done 回调
 */
ChannelWechatSchema.statics.getDefault = function (channelId, done) {
  ChannelWechat.findOne({ channel: channelId, default: true }).exec(function (err, o) {
    if(err) console.error(err);
    if(!o) {
      ChannelWechat.findOne({ channel: channelId }).exec(function (err, w) {
        if(err) console.error(err);
        return done(err, w);
      });
    }
...
ChannelWechatSchema.statics.getAuthWechat = function (channelId, done) {
  ChannelWechat.find({channel:channelId, type: { $in: ['3', '4'] } }).exec(function (err, wechats) {
    if(err) console.error(err);
    done(err, wechats);
  });
};
