...
var mongoose = require('mongoose')
  ...
  , ChannelPayment = mongoose.model('ChannelPayment')
 ...
 * @param channelPaymentId 渠道支付参数
 * @param cb
 * @returns {*}
 */
exports.sendPay = function(orderId, channelPaymentId, channel, ccbwebview, ip, cb) {
  if (!orderId) {
    return cb({'success': '0', 'msg': '订单不存在'});
  }
  if (!channelPaymentId && !channel) {
    return cb({'success': '0', 'msg': '支付失败，缺少渠道信息'});
  }
  //如果有支付方式，使用支付方式
  if (channelPaymentId) {
    ChannelPayment.findById(channelPaymentId).populate("certificate").exec((e, channelPayment)=>{
      if(e || !channelPayment){
        return cb({'success': '0', 'msg': '支付失败,支付参数不存在'});
      }
      processChannelPayment(orderId, channelPayment, ccbwebview, ip, function(data){
        cb(data);
      })
    })
  }else{
    //找渠道下的默认支付方式
    ChannelPayment.findOne({channelId : channel, default: true, enable: true}).populate("certificate").exec((e, channelPayment)=>{
      if(e || !channelPayment){
 ...
 * 按支付方式处理不同支付请求
 * @param orderId
 * @param channelPayment
 * @param ccbwebview
 * @param cb
 * @returns {*}
 */
function processChannelPayment(orderId, channelPayment, ccbwebview, ip, cb){
  var name = channelPayment.name;
  //记录订单关联的支付方式,并记录订单状态正在支付
  Order.findOneAndUpdate({"_id": orderId}, {"channelPayment": channelPayment._id, "paying": true}, (e,o)=>{});
  if(name.indexOf("微信") >= 0){
    wxPay(orderId, channelPayment, ip, (data)=>{
      cb(data)
    })
  }else if(name.indexOf("建行") >= 0){
    var wapver = getWapver(channelPayment);
    if(wapver == '1.2'){
      ccbPay_1_2(orderId, channelPayment, ccbwebview, function(data){
        cb(data)
      })
    }else if(wapver == '2.0'){
      ccbPay_2_0(orderId, channelPayment, function(data){
        cb(data)
      })
    }else{
...
function wxPay(orderId, channelPayment, ip, cb){
  Order.findOne({"_id": orderId, "status": "01", paid: false}).populate("member").exec(function(err, order) {
    ...
    if(!channelPayment.certificate){
      return cb({'success': '0', 'msg': '未找到支付证书'});
    }
    initConfig.pfx = fs.readFileSync(config.root + "/public" + channelPayment.certificate.url);
 ...
 * @param channelPayment
 */
function getWapver(channelPayment){
  var wapver = '';
  channelPayment.params.forEach(function(p) {
 ...
 * @param cb
 */
function ccbPay_1_2(orderId, channelPayment, ccbwebview, cb){
 ...
 * @param cb
 */
function ccbPay_2_0(orderId, channelPayment, cb){
  Order.findOne({"_id": orderId, "status": "01", paid: false}, function(err, order) {
    if (err || order == null) {
      return cb({'success': '0', 'msg': '订单不存在'});
    }else{
 ...
 * @param cb
 */
exports.genPayUrl2 = function(orderId, channel, cb){
  if(!orderId){
  ...
  Order.findOne({"_id": orderId, "status": "01"}, function(err, order) {
    if (err || order == null) {
      ...
      //从支付配置参数中获取参数信息
      ChannelPayment.findOne({"name": "建行支付", "channelId": channel, "enable": true}, function(e, payment){
        if(e || !payment){
          return cb({'success': '0', 'msg': '未找到相关支付参数'});
        }
        console.log('find channel payment=================>', payment)
        var config = {};
        payment.params.forEach(function(p){
          if(p.paramName == 'host'){
            config.host = p.paramValue;
 ...
 * @param cb
 */
exports.genPayUrlQrCode = function(orderId, channel, cb){
  if(!orderId){
    return cb({'success': '0', 'msg': 'orderId is null'});
  }
  Order.findOne({"_id": orderId, "status": "01"}, function(err, order) {
...
exports.pageCallback = function(req, res){
  ...
  if(result && result == 'Y'){
    ...
    //2.0支付
    Order.findOne({"no": ORDERID}).populate("channel").exec((err, orderInfo)=>{
      if(err || !orderInfo){
        res.render(__dirname + '../views/error', {error: {title: '无此订单:' + ORDERID}});
      }else{
        orderPayLogs.order = orderInfo._id;
        orderPayLogs.channel = orderInfo.channel._id;
        ...
        Setting.findOne({"key":"context.front"}, (e , setting)=>{
          ...
          if(orderInfo.no.indexOf('GX') == 0 || orderInfo.no.indexOf('TJ') == 0) {
            return res.redirect(absBaseUrl + '/scan/paySuccess?mobile='+ orderInfo.mobile+'&cid='+orderInfo.channel.identity+'&orderNo=' + orderInfo.no + '&phoneBill='+phoneBill + '&branch='+ orderInfo.branch);
          }else if(orderInfo.no.indexOf('YYGYN') == 0) {
            return res.redirect(absBaseUrl + '/buyone/paySuccess?mobile='+ orderInfo.mobile+'&cid='+orderInfo.channel.identity+'&orderNo=' + orderInfo.no);
    ...
    //    Order.findOne({"no": ORDERID}).populate("channel").exec(function(err, order){
    //      var phoneBill = 0;
    //      if(typeof order.phoneBill !== 'undefined' && order.phoneBill) {
    //        phoneBill = 1;
    //      }
    //      //返回商户页面
    //      res.redirect(req.absBaseUrl + '../../../../scan/paySuccess?mobile='+ order.mobile+'&cid='+order.channel.identity+'&orderNo=' + order.no + '&phoneBill='+phoneBill + '&branch='+ order.branch);
    //    })
    //  }
    //})
  }else{
    //返回商户页面
    Order.findOne({"no": ORDERID}).populate("channel").exec(function(err, orderInfo){
      orderPayLogs.order = orderInfo._id;
      orderPayLogs.channel = orderInfo.channel._id;
      OrderPayLogs.findAndSaveByOrderId(ORDERID, orderPayLogs, (e, o)=>{});
      var phoneBill = 0;
      ...
      Setting.findOne({"key":"context.front"}, (e , setting)=> {
        ...
        if(orderInfo.no.indexOf('GX') == 0) {
          return res.redirect(absBaseUrl + '/scan/payFail?mobile='+ orderInfo.mobile+'&cid='+orderInfo.channel.identity+'&orderNo=' + orderInfo.no + '&phoneBill='+phoneBill + '&branch='+ orderInfo.branch);
        }else if(orderInfo.no.indexOf('YYGYN') == 0) {
          return res.redirect(absBaseUrl + '/buyone/payFail?mobile='+ orderInfo.mobile+'&cid='+orderInfo.channel.identity+'&orderNo=' + orderInfo.no);
        }
      });
    })
  }
  ...
  if(result && result == 'Y'){
    Order.findOneAndUpdate({"no": ORDERID, "status": "01", "paid": false}, {"status": "02", "paid": true, "payTime": new Date()}, (e,o)=>{
      if(o != null){
        ...
        orderPayLogs.channel = o.channel;
        OrderPayLogs.findAndSaveByOrderId(ORDERID, orderPayLogs, (e, o)=>{});
      }
...
exports.pageCallback1_2 = function(req, res){
  ...
  if(result && result == 'Y'){
    ...
    //      phoneBill = 1;
    //    }
    //    console.log('redirect url============>', req.absBaseUrl + '../../../../scan/paySuccess?mobile='+ orderInfo.mobile+'&cid='+orderInfo.channel.identity+'&orderNo=' + orderInfo.no + '&phoneBill='+phoneBill + '&branch='+ orderInfo.branch)
    //    res.redirect(req.absBaseUrl + '../../../../scan/paySuccess?mobile='+ orderInfo.mobile+'&cid='+orderInfo.channel.identity+'&orderNo=' + orderInfo.no + '&phoneBill='+phoneBill + '&branch='+ orderInfo.branch);
    //  }
    //})
    ...
    Order.findOne({"no": ORDERID}).populate("channel").exec(function(err, orderInfo){
      ...
      res.redirect(req.absBaseUrl + '../../../../../scan/payFail?mobile='+ orderInfo.mobile+'&cid='+orderInfo.channel.identity+'&orderNo=' + orderInfo.no + '&phoneBill='+phoneBill + '&branch='+ orderInfo.branch);
    })
  }
}
